...
It was developed for the '[CASA Summer School](https://bonnie-buyuklieva.github.io/SS_ASM19/)', or the Doctoral Summer School for Advanced Spatial Modelling: Skills Workshop and Hackathon, 21^st^ to 23^rd^ August 2019, for it's full name!
...
The 'too long didn't read' (TLDR) take away messages from this lecture are:
...
- **there is an officially supported code, called an EPSG code, for most places --- that is often a good option**
...
By default, most software (including R) uses a cylindrical projection, leading to maps like this:
...
plot(london$geometry, add = TRUE, pch = 9)
```
...
plot(zones_london_projected$geometry)
plot(london_projected$geometry, add = TRUE, pch = 19, cex = 9)
```
