...
```{r}
d = readr::read_csv("https://github.com/Robinlovelace/geocompr/releases/download/1.2/COVID19-data-2020-04-20.csv")
names(d)
head(d)
```
...
head(world)
plot(world)
summary(d$country %in% world$name_long)
unique(d$country[!d$country %in% world$name_long])
unique(d$country[!d$country %in% world$name_long])
```
...
```{r, eval=FALSE}
# Fails due to issue with date conversion
tm_shape(g) +
  tm_lines(col = "grey") +
  tm_shape(w_today) +
...
```{r, eval=FALSE}
# Fix issue with date conversion
w$Date = as.character(w$date)
plot(w$date, w$deaths)
tm_shape(g) +
  tm_lines(col = "grey") +
  tm_shape(w_today) +
  tm_polygons(c("deaths"), palette = "viridis") +
