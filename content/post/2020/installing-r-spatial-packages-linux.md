...
This post will be updated in late April when some of the dust has
settled around these changes.
...
distributions, Ubuntu (covered in this post) in particular. The guidance
in this post should get geographic R packages set-up quickly in a
future-proof way, but the only way to ensure you have full control over
...
This take time but, building on [OSGeo](https://www.osgeo.org/)
libraries, a well set-up Linux machine is an ideal platform to install,
run and develop key geographic R packages in a performance, stable and
future-proof way.
